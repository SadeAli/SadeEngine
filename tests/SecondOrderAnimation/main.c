...
#define MAX 100
    ...
    // create animation
    Vector2 p[] = {(Vector2){0, 10}, (Vector2){50, 100}, (Vector2){100, 190}, (Vector2){0, 280}};
    const int steps = 500;
    AnimationCurve ac = getAC(p, steps);
    ...
    SecondOrderSystem x = INIT_SECOND_ORDER_SYSTEM(-5, 4, 0.2);
    SecondOrderSystem y = INIT_SECOND_ORDER_SYSTEM(-5, 4, 0.2);
    ...
    while (!WindowShouldClose())
    {
        ...
        solveSecondOrderSystem(&x, targetPos.x, targetDelta.x, GetFrameTime());
        solveSecondOrderSystem(&y, targetPos.y, targetDelta.y, GetFrameTime());
        pba = (Vector2){x.y, y.y};
